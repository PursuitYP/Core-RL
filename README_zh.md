...
当前最强主线是 `Scale-Invariant Continuing Control`，它把 reward centering 和 output-controlled Sarsa 放进同一个 continuing access-control task，研究在线控制 agent 能否在 reward origin 和 feature unit 变化时仍保持稳定。
...
`Reward-Centered Continuing Sarsa` 和 `Output-Controlled TD` 是支撑 integrated invariance 方向的强机制研究。`Predictive State Plasticity` 当前是有价值的 negative/redesign gate：在 T-maze control 中 learned GVF state 仍接近 chance，而 trace/oracle memory 能解决任务。
...
当前多角色审查记录在 `final/indexes/reviewer_audit.md` 和 `final/indexes/reviewer_audit_zh.md`。这些文件记录审查者提出的问题、已经采取的行动和仍待处理的事项，例如扩展中文报告、加入图表再生成命令、把 extended results 写回报告等。
