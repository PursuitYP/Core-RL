...
| Output-Controlled TD | Strong; 20-seed extended + fairness audit | True-online output control still needs a more principled derivation; no no-reset feature-scale switch. | Max-stable-alpha audit; principled true-online normalization; scale switch without reset. | Keep fairness evidence in the report and add switch-result sections when available. |
