...
This document preserves the current major revision plan for review and continuation. The confirmed scope is now all 17 formal research objects: 13 ordinary proposals and 4 integrated proposals. Each proposal remains an independent research study. The integrated proposals are larger and more synthetic, but they do not replace the ordinary proposal reports.
...
The final-facing structure is now `final/reports/proposals/` for the 13 ordinary reports and `final/reports/integrated/` for the 4 larger reports. Each formal report folder should use `report.md`, `report_zh.md`, and `report.pdf` as the main entry points. `final/archive/` is audit-only.

The next long CPU task `core-rl-scale-invariant-extended-33723554` has now succeeded. It ran `scale_invariant_continuing_control/config_extended.json` on `ailab-safethm/safethm_cpu_task` and wrote standard artifacts to `experiments/alberta_core_rl/results/scale_invariant_continuing_control/20260709T063128Z_extended`. This directory is now evidence and has been incorporated into the Scale-Invariant report, figures, and indexes.

The second-round reward-centered sensitivity and Dyna drift CPU reruns have also produced standard artifacts and are now evidence. Reward sensitivity is at `experiments/alberta_core_rl/results/reward_centered_sarsa_sensitivity/20260709T085747Z_extended`, with 10 seeds, 8,112,150 metric rows, and 1,575 condition groups. Dyna drift is at `experiments/alberta_core_rl/results/continual_dyna_model_aging_drift/20260709T085809Z_extended`, with 10 seeds configured, 1,800,900 metric rows, and 690 condition groups. The larger Useful Predictive Knowledge proposal has been implemented through a first gate, with report path `final/reports/integrated/useful_predictive_knowledge/report.md` and result path `experiments/alberta_core_rl/results/useful_predictive_knowledge/20260709T102402Z_main`.

## Honest Gap Statement

The previous passes completed important infrastructure, evidence synchronization, and several targeted extended sweeps. This revision adds the missing first feature-budget stage for Useful Predictive Knowledge, so the representation line now has decodability/control-usefulness evidence and a two-feature budget-selection gate. The full user request is still broader than this checkpoint: several weak proposals have only 5-seed pilot evidence or one diagnostic figure; English and Chinese reports are paired but not guaranteed to be paragraph-by-paragraph identical; multi-role critique exists in each report but is still concise; and the Useful Predictive Knowledge proposal still lacks oracle-prediction scaling, explicit prediction-to-policy coupling, and plasticity under phase switches. The current state is therefore a materially improved intermediate state, not a completed final revision.

## Requirement Traceability Matrix

| Original requirement | Current reality | Gap | Required action | Acceptance criterion |
|---|---|---|---|---|
| 0. Every proposal is independent | The directory structure has 13 ordinary reports and 4 integrated reports. | Some reports still need deeper paper-style storylines and richer evidence, but they now have explicit template/scope/evidence/audit structure. | Continue deepening `Proposal Template Answers`, `Independent Research Scope`, `Evidence Level`, `Required Next Experiments`, and `Reviewer Audit` across all 17 reports. | Any single report can be read alone to understand question, setting, methods, evidence, limits, and next steps. |
...
| 2. Combine into larger meaningful questions without patchwork | Four integrated proposals now exist, including Useful Predictive Knowledge. | The new representation topic now has Gate-1/Gate-2 evidence plus a Gate-3 feature-budget selection result, but plasticity and oracle-prediction scaling remain open. | Keep the four integrated proposals; deepen Useful Predictive Knowledge with oracle-prediction scaling, prediction-to-policy coupling, and plasticity experiments. | The large topic has one motivation, one environment family, staged experiments, and explicit success/failure criteria. |
...
1. Evidence and index cleanup: remove stale statements that Output-Controlled TD, Reward Sensitivity, Dyna Drift, or Scale-Invariant are still running; update `results*.md`, `status*.md`, `reviewer_audit*.md`, and both proposal overviews whenever a result directory becomes evidence.
...
4. Experiment expansion: Reward-Centered beta/gamma/no-reset switch, Reward-Centered early recovery analysis, Output-Controlled fairness audit plus max-stable-alpha frontier, full fixed-condition scale grid, Dyna abrupt/gradual/stochastic drift plus reward/staleness frontier, and Useful Predictive Knowledge feature-budget selection are complete first extensions. Remaining targeted experiments are Reward-Centered matched-seed recovery/policy-distance probes, Output-Controlled no-reset feature-scale switch and principled true-online derivation, Scale-Invariant gradual unit drift, Dyna repeated changes and true per-backup planning-utility logging, Useful Predictive Knowledge oracle-prediction scaling/prediction-to-policy coupling/plasticity gates, Options fixed-goal sanity, and canonical TIDBD/AutoStep or demotion.
...
|---|---|---|---|---|
| Reward-Centered Sarsa | Strong; 20-seed fixed-shift extended + completed 10-seed sensitivity/no-reset switch + early recovery analysis | Needs richer paired recovery and policy-use analysis beyond value-scale diagnostics. | Add matched-seed recovery, policy-distance probes, and natural reward drift. | Make reward-origin invariance the central question, not just higher reward. |
| Output-Controlled TD | Strong; 20-seed extended + fairness audit + max-stable-alpha frontier | True-online output control still needs a more principled derivation; no no-reset feature-scale switch. | Principled true-online normalization; scale switch without reset. | Keep fairness evidence in the report and add switch-result sections when available. |
...
| Scale-Invariant Continuing Control | Strong integrated; fixed grid + unit-switch complete | Abrupt switch recovery remains weak despite fixed-grid stability. | Add gradual unit drift, recovery AUC, and policy-distance probes. | Present one coherent reward-unit and feature-unit invariance question. |
| Continual Dyna Model Aging | Strong integrated; fixed-change extended + completed abrupt/gradual/stochastic drift + reward/staleness frontier | Stochastic drift weakens universal aging claims; repeated natural changes remain open. | Add repeated changes, clearer stochastic transition families, and true per-backup planning-utility logging. | Focus on conditional stale-backup reduction and freshness-aware search control. |
...
| Useful Predictive Knowledge | New integrated gate; completed Gate-1/Gate-2 T-maze result and Gate-3 feature-budget result | Learned cue predictions are decodable and can be selected under a two-feature budget, but they are not yet control-useful; plasticity and oracle-prediction scaling are not implemented. | Add oracle-prediction control, GVF-output normalization, explicit policy-use ablations, and later plasticity. | Present prediction usefulness as staged gates, not as a generic GVF-success claim. |

## New Larger Proposal Candidate Now Active

Active candidate: `Useful Predictive Knowledge Under Partial Observability and Resource Limits`. This does not replace the other 16 reports; it is a fourth integrated proposal meant to satisfy the requirement to break out of the current fragmented frame without creating a patchwork. The focused question is: when do learned predictions become useful agent state under partial observability and limited feature/computation budgets?

Unified environment family: T-maze cue stream plus delayed-cue trace-conditioning stream. Unified object of study: whether hidden cue information can be retained until control-relevant time by GVFs, trace features, utility-selected features, or per-feature step-size adaptation. Unified metrics: cue decodability, junction action accuracy, trial reward, feature-budget usage, prediction TD error, and post-switch recovery. The first T-maze gate is complete at `experiments/alberta_core_rl/results/useful_predictive_knowledge/20260709T102402Z_main`: learned cue predictions are above chance in decodability but do not yet improve control. The first feature-budget gate is complete at `experiments/alberta_core_rl/results/useful_predictive_knowledge_budget/20260709T132025Z_main`: selectors can keep cue-GVF features, but control remains near chance. Remaining staged experiments are oracle-prediction scaling, GVF-output normalization, policy-use coupling, and only then TIDBD/AutoStep-style plasticity. Failure is informative: if prediction error drops or cue features are selected but control does not improve, the conclusion is that prediction accuracy and selection are not sufficient for useful state.
...
| Monitor Scale-Invariant extended CPU task | Done | `core-rl-scale-invariant-extended-33723554` succeeded; `20260709T063128Z_extended` contains standard artifacts and is incorporated. | Continue with gradual drift and recovery diagnostics. |
...
| Deepen strong proposal experiments | Partially done | Output-Controlled extended evidence, Reward-Centered sensitivity, Dyna drift, Scale-Invariant fixed grid, and Useful Predictive Knowledge Gate-1/Gate-2/Gate-3 are incorporated. | Prioritize targeted sweeps that change conclusions rather than broad blind grids. |
| New large predictive-knowledge proposal | Active first and budget gates complete | `final/reports/integrated/useful_predictive_knowledge/`, `experiments/alberta_core_rl/results/useful_predictive_knowledge/20260709T102402Z_main`, and `experiments/alberta_core_rl/results/useful_predictive_knowledge_budget/20260709T132025Z_main` exist. | Add oracle-prediction scaling, GVF-output normalization, prediction-to-policy coupling, and later plasticity stages. |
