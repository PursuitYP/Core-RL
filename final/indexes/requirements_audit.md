...
| Prioritize Reward-Centered Sarsa and Output-Controlled TD | Covered with implementation, reports, figures, and extended evidence | `final/reports/proposals/reward_centered_sarsa/`, `final/reports/proposals/output_controlled_td/` | Reward-Centered now has fixed-shift, beta/gamma/no-reset switch, and early recovery evidence; Output-Controlled now has primary and fairness-audit evidence. Remaining gaps are Reward matched-seed recovery/policy-distance probes and Output no-reset feature-scale switch. |
...
| New larger integrated Core RL proposals | Covered as a first integrated pass plus targeted Useful budget extension | `final/reports/integrated/`, `final/reports/integrated/useful_predictive_knowledge/` | Four integrated reports now exist. Useful Predictive Knowledge has a report, Gate-1/Gate-2 result, and Gate-3 feature-budget result; oracle-prediction scaling and plasticity stages remain open. |
...
| Current CPU-task tracking | Covered for current evidence | `final/indexes/status.md`, `final/indexes/revise_plan_20260709.md` | Scale-Invariant, Output-Fairness, Reward-Sensitivity, and Dyna-Drift extended jobs have succeeded and are evidence. The remaining gap is keeping future CPU-task status from drifting across duplicated documents. |
...
1. Continue deepening Proposal Template Answers, independent scope, evidence level, required next experiments, and reviewer-audit sections across all 17 current reports.
...
6. Keep PDFs and image-link manifests synchronized after future report edits; the latest 17-report export has been regenerated with `missing_images=0`.
