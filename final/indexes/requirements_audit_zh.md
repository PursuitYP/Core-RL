...
| 重点实现 Reward-Centered Sarsa 和 Output-Controlled TD | 已覆盖，并已有实现、报告、图表和 extended evidence | `final/reports/proposals/reward_centered_sarsa/`、`final/reports/proposals/output_controlled_td/` | Reward-Centered 现在已有 fixed-shift、beta/gamma/no-reset switch 和 early recovery evidence；Output-Controlled 现在已有 primary 和 fairness-audit evidence。剩余缺口是 Reward matched-seed recovery / policy-distance probes 和 Output no-reset feature-scale switch。 |
...
| 新的更大 integrated Core RL proposal | 第一轮 integrated pass 加 targeted Useful budget extension 已覆盖 | `final/reports/integrated/`、`final/reports/integrated/useful_predictive_knowledge/` | 现在已有四个 integrated reports。Useful Predictive Knowledge 已有 report、Gate-1/Gate-2 result 和 Gate-3 feature-budget result；oracle-prediction scaling 与 plasticity stages 仍未完成。 |
...
| 当前 CPU task 追踪 | 当前 evidence 层面已覆盖 | `final/indexes/status_zh.md`、`final/indexes/revise_plan_20260709_zh.md` | Scale-Invariant、Output-Fairness、Reward-Sensitivity 和 Dyna-Drift extended jobs 都已成功并作为 evidence。剩余缺口是未来 CPU-task 状态不要在重复文档中漂移。 |
...
1. 继续深化 17 个当前报告的 Proposal Template Answers、独立研究范围、证据等级、下一步实验和 reviewer-audit sections。
...
6. 后续每次修改报告后继续同步 PDF 和图片链接 manifest；最新 17-report export 已重新生成，且 `missing_images=0`。
