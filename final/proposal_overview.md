...
2. Continual Dyna With Model Aging. This is the strongest planning/model-based direction. It asks which model entries deserve planning after the world changes. Current 20-seed half-life/budget evidence shows recency aging sharply reduces stale backups, especially at planning budget `20`. The gradual/stochastic drift extension has a smoke result and a running CPU extended job; it should be incorporated only after standard artifacts appear.

3. Reward-Centered Continuing Sarsa. This is a clean standalone main proposal. It asks whether continuing Sarsa should be invariant to arbitrary reward offsets. Current access-control evidence shows ordinary discounted Sarsa inflates value scale under reward shifts, while centered and differential variants remain more stable. The beta/gamma/no-reset reward-origin switch extension has a smoke result and a running CPU extended job; it is pending evidence, not a final claim yet.

4. Output-Controlled TD. This is the clean standalone prediction/function-approximation proposal. It asks whether alpha should control prediction change rather than raw parameter change. Current citable tile-coded random-walk evidence is the 20-seed extended CPU-task run `experiments/alberta_core_rl/results/output_controlled_td/20260709T051934Z_extended`: normalized TD and trace-normalized TD have `0/400` seed-conditions diverged, while fixed TD and the raw-alpha true-online baseline each diverge in `141/400` seed-conditions. The completed normalized true-online fairness audit `experiments/alberta_core_rl/results/output_controlled_td_fairness_audit/20260709T085746Z_extended` shows normalized TD and trace-normalized TD(lambda) at `0/300`, fixed and raw-alpha true-online at `81/300`, and naive normalized true-online at `34/300`, with failures concentrated in lognormal scaling.
...
All current results use online interaction and seed-aware tail summaries in `condition_summary.json`. Reward-Centered Sarsa, Output-Controlled TD, Output-Controlled TD Fairness Audit, Dyna Planning Budget, Scale-Invariant Continuing Control, Continual Dyna Model Aging, Predictive State Plasticity, and Unit-Switching Continuing Control now have extended evidence. Reward-Centered sensitivity and Dyna drift still have running CPU jobs or pending extended directories that are not evidence until the standard artifacts are written. Report-ready figures are generated with `experiments/alberta_core_rl/scripts/plot_report_figures.py`; rjob-owned result directories are handled by writing figures into the relevant report folders.
