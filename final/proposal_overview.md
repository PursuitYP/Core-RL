...
4. Output-Controlled TD. This is the clean standalone prediction/function-approximation proposal. It asks whether alpha should control prediction change rather than raw parameter change. Current citable tile-coded random-walk evidence is now the 20-seed extended CPU-task run `experiments/alberta_core_rl/results/output_controlled_td/20260709T051934Z_extended`: normalized TD and trace-normalized TD have `0/400` seed-conditions diverged, while fixed TD and the current raw-alpha true-online baseline each diverge in `141/400` seed-conditions. Next step: audit the true-online baseline with a fair max-stable-alpha comparison and add a nonstationary feature-scale switch.
...
All current results use online interaction and seed-aware tail summaries in `condition_summary.json`. Reward-Centered Sarsa, Output-Controlled TD, Dyna Planning Budget, Continual Dyna Model Aging, Predictive State Plasticity, and Unit-Switching Continuing Control now have 20-seed extended evidence. Scale-Invariant Continuing Control still needs the full fixed-condition extended grid. Report-ready figures are generated with `experiments/alberta_core_rl/scripts/plot_report_figures.py`; Output-Controlled TD extended figures are stored in its report folder because the rjob-owned result directory is not writable.
